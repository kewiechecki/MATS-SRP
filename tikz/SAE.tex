\begin{tikzpicture}[
  ...
  \node[label=above:input data] (X) {$E_{outer}^{m \times n}$} ;
  \node[punkt] (encoder) [below=of X] {$\encoder_{inner}^{m \to d}$} ;
  \node[label=right:embeddings] (E) [below=of encoder] {$E_{inner}^{d \times n}$} ;
  ...
  \node[label=left:soft clusters] (K) [below=of softmax] {$K_{inner}^{k \times n}$} ;
  ...
  \node[label=right:denoised embeddings] (Ehat) [below=of GE] {$\hat{E}_{inner}^{d \times n}$} ;

  \node[punkt] (decoder) [left=of Ehat] {$\decoder_{inner}^{d \to m}$} ;
  \node[label=above:reconstructed data] (Xhat) [left=of decoder] {$\hat{E}_{outer}^{m \times n}$} ;
